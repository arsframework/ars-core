...
public final class Files {
    ...
    public static String getName(String path) {
        ...
        int division1 = path.lastIndexOf("\\");
        int division2 = path.lastIndexOf('/');
        if (division1 > -1 && division2 > -1) {
            return path.substring((division1 > division2 ? division1 : division2) + 1);
        } else if (division1 > -1) {
            return path.substring(division1 + 1);
        } else if (division2 > -1) {
            return path.substring(division2 + 1);
        }
        return path;
    }
    ...
    public static String getDirectory(String path) {
        ...
        int division2 = path.lastIndexOf('/');
        if (division1 > -1 && division2 > -1) {
            return path.substring(0, division1 > division2 ? division1 : division2);
        } else if (division1 > -1) {
            return path.substring(0, division1);
        } else if (division2 > -1) {
            return path.substring(0, division2);
        }
        return null;
    }
     ...
     * @return true/false
     */
    public static boolean isSuffix(String path, String... suffixs) {
        if (Strings.isEmpty(path) || suffixs == null || suffixs.length == 0) {
            return false;
        }
        ...
        for (String _suffix : suffixs) {
            if (suffix.equalsIgnoreCase(_suffix)) {
                return true;
            }
        }
        return false;
    ...
    public static String getUnitSize(long size) {
        if (size < 0) {
            throw new IllegalArgumentException("Size must not be less than 0, got " + size);
        }
        StringBuilder buffer = new StringBuilder();
        if (size >= 1073741824) {
            buffer.append(Beans.DEFAULT_DECIMAL_FORMAT.format(size / 1073741824d)).append("GB");
        } else if (size >= 1048576) {
            buffer.append(Beans.DEFAULT_DECIMAL_FORMAT.format(size / 1048576d)).append("MB");
        } else if (size >= 1024) {
            buffer.append(Beans.DEFAULT_DECIMAL_FORMAT.format(size / 1024d)).append("KB");
        } else {
